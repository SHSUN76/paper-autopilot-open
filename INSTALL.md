...
1. **사전 점검** — node/npm/git/pandoc + Fable 5 접근
2. **의존성 설치** — `scripts/`의 Node 패키지
3. **config 마법사** — `~/.claude/paper-autopilot-open/config.json` 대화형 작성
4. **corpus 구축** — 본인 논문 ~5편 + 분야 논문 ~5편 → 로컬 RAG
...
| Python 3.10+ | 선택 | docx 변환·figure 추출용 (pip: `PyMuPDF`, `Pillow`, `python-docx`) |
| Playwright MCP | 선택 | `paper-access` 기관 구독 접근(Tier 1/2) |
...
본인 논문·분야 논문 PDF를 `paper-corpus-mining` 스킬로 태깅해 `paragraph_reports/*.json`을 만든 뒤:
...
플래그: `--input <dir>` `--group own|field` `[--force]`. 비용: Gemini 무료 티어면 $0, OpenAI면 10편 기준 <$0.5.
...
The onboarding wizard runs 6 phases: pre-check (node/npm/git/pandoc + Fable 5 access) → install deps → config wizard (`~/.claude/paper-autopilot-open/config.json`) → build a local RAG corpus from your own ~5 papers + ~5 field papers → verify → summary.

**Required:** latest Claude Code with Fable 5 access, Node.js 18+, git, a Gemini API key.
**Optional:** OpenAI / Anthropic / STORM / Tavily keys, pandoc (docx), Playwright MCP (paper-access), a Supabase project (`rag.mode=supabase`).

**Manual path (advanced):** copy `config/settings.template.json` to `~/.claude/paper-autopilot-open/config.json` and fill it, run `npm ci` in `scripts/`, then `build-corpus.mjs --input <dir> --group own|field`.
