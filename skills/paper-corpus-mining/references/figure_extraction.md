...
아래 필드명은 정본 계약이다 — 이름을 바꾸지 말 것. **같은 스키마(figures 배열 + 아크 필드 + 아래 optional `methodology` 블록)가 `skills/onboarding/references/corpus-build.md` 2.5–2.6에도 있으며 두 파일의 필드명은 동일해야 한다**(한쪽만 수정 금지).
...
`figures`·아크 필드와 **나란히(top-level 형제로)** optional `methodology` 블록을 함께 담을 수 있다 — 논문이 쓴 분석 기법의 **사용 맥락**(기법 → 목적 → 증명 대상 → 짝지은 figure)을 구조화한다. 스키마·판별 기준은 아래 **"methodology 블록"** 절 참조. 없으면 생략 가능(optional).
...
## methodology 블록 (optional top-level)

figure 구성뿐 아니라 **방법론(고도분석 기법)의 사용 맥락**을 RAG에 담기 위한 optional top-level 블록이다. `<paper_id>.figures.json`의 `figures`·아크 필드와 나란히 둔다. 핵심은 기법을 **나열**하는 것이 아니라 **"무엇을 증명하려고 이 기법을 썼는가"**를 논문의 claim과 연결하는 것이다.

```json
"methodology": {
  "techniques": [{
    "technique": "operando XRD",
    "category": "advanced",
    "purpose": "충방전 중 상전이 실시간 추적",
    "evidence_target": "mechanism: 용량 감쇠가 상전이 비가역성에서 온다는 주장의 직접 근거",
    "figures": ["Fig3"],
    "instrument_notes": "synchrotron beamline 등 (optional)"
  }],
  "analysis_pipeline": "1-2문장: 논문의 분석 전략 흐름"
}
```

### 필드별 가이드

- `techniques[]` — 논문이 사용한 분석 기법 배열. **논문에 명시된 기법만** 담는다(추정 금지).
  - `technique` — 기법 이름(예: `operando XRD`, `cryo-TEM`, `XPS depth profiling`, `ToF-SIMS`).
  - `category` — `advanced` 또는 `standard` 중 하나(아래 판별 기준표).
  - `purpose` — 이 기법으로 **무엇을 보려 했는지**. 본문에 근거가 있어야 한다(hallucination 금지).
  - `evidence_target` — 이 기법이 **논문의 어떤 claim을 증명하는 직접 근거**인지. `mechanism:`·`performance:`·`structure:` 등 claim 유형을 앞에 붙여 논문 주장과 연결한다. 이 필드가 methodology RAG의 핵심 검색 대상이다.
  - `figures[]` — 이 기법 결과가 실린 figure의 `fig_id` 배열(예: `["Fig3"]`). Main·SI 모두 표기.
  - `instrument_notes` — (optional) 장비·빔라인·조건 등 부가 정보(예: `synchrotron beamline`, `Ar⁺ etching 5 nm/step`). 없으면 생략.
- `analysis_pipeline` — 논문의 **분석 전략 흐름**을 1-2문장으로. 어떤 순서로 기법을 엮어 결론에 도달했는지.

### category 판별 기준표

| category | 해당 기법 (예시) |
|----------|------------------|
| `advanced` | operando/in-situ 측정, synchrotron 기반 기법, cryo-EM, depth profiling(XPS etching·ToF-SIMS), 시간분해(time-resolved) 분광, isotope tracing, 다중기법 correlative 분석 등 |
| `standard` | 통상 XRD/SEM/TEM/CV/EIS/GCD 등을 **단독**으로 통상 조건에서 사용 |

경계가 애매하면 "통상 조건 단독 사용인가(=standard) / 특수 조건·시간분해·다중기법 결합인가(=advanced)"로 판정하고, 근거를 `purpose`에 남긴다.

### Hallucination 가드 (methodology 전용)

- **논문에 명시된 기법만** `techniques[]`에 담는다. 그림만 보고 기법을 추정하지 않는다.
- `purpose`·`evidence_target`은 **본문 근거**가 있어야 한다 — 저자가 그 기법으로 그 주장을 뒷받침한다고 실제로 서술한 경우에만.
- 기법이 어느 figure에도 연결되지 않으면 `figures`를 빈 배열(`[]`)로 두되, `analysis_pipeline`에 맥락을 남긴다.
...
6. **methodology 구성** (있으면) — 본문에서 저자가 명시한 분석 기법을 훑어 `methodology.techniques[]`를 채운다. 각 기법의 `category`(advanced/standard), `purpose`, **claim과 연결한 `evidence_target`**, 짝지은 `figures`를 적고, 전체 `analysis_pipeline`을 1-2문장으로 판정한다. 명시된 기법이 없으면 이 블록은 생략한다.
...
- [ ] 불확실한 해석은 `(불확실)`로 표기됐다.
- [ ] (methodology가 있으면) 각 technique의 `category`가 advanced/standard 중 하나이고, `evidence_target`이 논문 claim과 연결됐으며, 논문에 명시된 기법만 담겼다.
