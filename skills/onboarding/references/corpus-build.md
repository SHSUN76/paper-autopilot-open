...
## 2.6 methodology 블록 (optional top-level)

figure 구성뿐 아니라 **방법론(고도분석 기법)의 사용 맥락**(기법 → 목적 → 증명 대상 → 짝지은 figure)을 RAG에 담기 위해, `<paper_id>.figures.json`에 `figures`·아크 필드와 **나란히(top-level 형제로)** optional `methodology` 블록을 담을 수 있다. own+field 논문에만 해당하며(review 제외), 논문에 명시된 기법이 없으면 생략한다.
...
- `category`는 `advanced`(operando/in-situ, synchrotron, cryo-EM, depth profiling(XPS etching·ToF-SIMS), 시간분해 분광, isotope tracing, 다중기법 correlative 등) 또는 `standard`(통상 XRD/SEM/TEM/CV/EIS/GCD 단독 사용) 중 하나.
- 핵심은 기법 나열이 아니라 **`evidence_target`을 논문의 claim과 연결**하는 것(예: `mechanism: …의 직접 근거`).
- **hallucination 가드**: 논문에 명시된 기법만 담고, `purpose`·`evidence_target`은 본문 근거가 있어야 한다.
- `build-corpus.mjs`는 이 블록을 methods 인덱스로 적재하고 `retrieve.mjs methods`로 검색한다(아래 3.7).

판별 기준표·필드별 상세 가이드는 `skills/paper-corpus-mining/references/figure_extraction.md`의 **"methodology 블록"** 절 참조.
...
동작: config의 `embedding.provider`로 각 문단을 임베딩(`dimensions`=3072) → `rag.local_corpus_dir`(기본 `~/.claude/paper-autopilot-open/corpus`)에 벡터스토어로 저장. own/field 그룹에 `.figures.json`이 함께 있으면 figure 인덱스(`figures.jsonl`)와 아크(`figure-arcs.json`)도 갱신하고, `.figures.json`에 optional `methodology` 블록이 있으면 methods 인덱스로도 적재한다. 진행 로그는 stderr, 요약 JSON은 stdout:

```
{ papers_added, papers_skipped, paragraphs_embedded, moves_added,
  vocabulary_added, aitells_added, figures_added, arcs_added, methods_added,
...
# 방법론(고도분석 기법) 사용 맥락 검색: query로 유사 기법-증명 맥락을 찾고, technique/category/group으로 필터
node "${CLAUDE_PLUGIN_ROOT}/scripts/retrieve.mjs" methods --query "phase transition during cycling" --technique "operando XRD" --category advanced --group field --k 5
```
...
- `methods` 플래그: `--query "..."`(의미 검색 — `purpose`·`evidence_target` 대상), `--technique <name>`(기법명 필터), `--category advanced|standard`(기법 등급 필터), `--group own|field`, `--k N`(반환 개수). 적재된 methodology 블록의 기법을 검색해 "어떤 기법을 무엇을 증명하려고 썼는지"를 반환한다. 새 논문의 방법론 설계·figure 짝짓기 시 exemplar로 쓴다.
