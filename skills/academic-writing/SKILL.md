...
| Methodology | `methodology.jsonl` — per-technique records (`technique`, `category` standard\|advanced, `purpose`, `evidence_target`, `figures[]`, `analysis_pipeline`, `similarity`) — 기법→목적→증명대상(claim)→짝지은 figure 매핑 | `CorpusMethodology` |
...
| Query CLI | `scripts/retrieve.mjs` — commands: `paragraphs`, `next-paragraph`, `vocabulary`, `aitells`, `section-distribution`, `move-transitions`, `style-profile`, `field-profile`, `figures`, `figure-arcs`, `methods`; options on `paragraphs`: `--group own\|field\|review`, `--since <year>` (local only); `figures` options: `--query`, `--type <figure_type>`, `--role <narrative_role>`, `--group own\|field`, `--k`; `figure-arcs` options: `--group field` (전체 아크 배열 반환); `methods` options: `--query`, `--technique <부분일치>`, `--category standard\|advanced`, `--group own\|field\|review`, `--k` | same (`--group` / `--since` ignored in supabase mode) |
...
4. **Methodology RAG** — 논문별 "기법 → 목적 → 증명 대상(claim) → 짝지은 figure" 매핑을 `methods` 커맨드로 조회: mechanism/성능 주장을 분야가 어떤 기법으로 증명하는지(관례)를 figure 설계(ra-figure-set)·SOP(ep-sop-writer)·기술 리뷰(aw-technical-reviewer)·기전 해석(ra-results-discussion)이 참조. methodology RAG 미구축 시 커맨드가 exit 1 → 에이전트는 기존 방식으로 폴백.
